...
export const IngredientsList = (props) => {
  ...
  const addIngredient = () => {
    ...
    const existingIngredientWithMatchingName = ingredients.find((ingredient) => {
  ...
  const deleteIngredient = (ingredientToRemove) => {
    const ingredientsWithRemovedItem =
  ...
  return (
    <div>
      ...
      <label>
        ...
        <Input value={additionalIngredientName} placeholder="Ingredient name" onChange={(event) => setAdditionalIngredientName(event.target.value)}/>
        <Button secondary onClick={(event) => {event.preventDefault(); addIngredient() } }>Add</Button>
      </label>
        ...
        <ul>
          {
            ingredients.map((ingredient) =>
                <li key={ingredient.name}>
                  <p>{ingredient.name} - <ButtonLink onClick={(event) => {event.preventDefault(); deleteIngredient(ingredient) } }>Remove</ButtonLink></p>
                </li>
            )
          }
        </ul>
